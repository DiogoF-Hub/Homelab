...
- `scripts/root_scripts/lib.sh` `emit_status` , printf-built JSON (no jq dep to emit), one line per phase to `/srv/logs/status/vault-maint-status-YYYY-MM-DD.jsonl` (`event_type=vault_maint`, `phase`, `status`, `rc`, + detail). `status_init` arms an EXIT trap so every phase reports even on an unexpected exit.
...
- `maintenance/vault-maint-agent.localfile.xml` , the Vault VM agent `<localfile>`. The log is dated daily, so the `<location>` uses an strftime path (`...-%Y-%m-%d.jsonl`); Wazuh's logcollector resolves the current file per day.
